...
export default function EditStoryPage() {
  ...
  const fetchStory = async (token: string) => {
    try {
      const { data } = await axios.get(`http://localhost:3002/api/admin/stories/${storyId}`, {
  ...
  const handleSubmit = async (status: 'DRAFT' | 'PUBLISHED') => {
    ...
    try {
      await axios.put(
        `http://localhost:3002/api/admin/stories/${storyId}`,
        { ...formData, status },
        {
          headers: {
      ...
      alert(error.response?.data?.error || error.message || 'Failed to update story');
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="space-y-4">
                ...
                {formData.label && !LABEL_OPTIONS.includes(formData.label) && (
                  <div className="text-sm text-gray-600">
                    Current: <span className="font-medium text-blue-600">{formData.label}</span>
                  </div>
                )}
              </div>
            </CardContent>
